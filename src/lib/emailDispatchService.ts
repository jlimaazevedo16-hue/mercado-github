...
// Core dispatch function - can be used without hooks
export async function dispatchEmail(
  ...
  try {
    ...
    if (!config) {
      await logEmailDispatch(params, 'erro', 'Configuração de e-mail não encontrada', null, userId);
      return { success: false, error: 'Configuração de e-mail não encontrada', logged: true };
    }

    if (!config.envio_ativo) {
      await logEmailDispatch(params, 'desativado', 'Envio de e-mail desativado nas configurações', null, userId);
    ...
    if (error) {
      await logEmailDispatch(params, 'erro', error.message, null, userId);
      return { success: false, error: error.message, logged: true };
    }

    if (!data?.success) {
      await logEmailDispatch(params, 'erro', data?.error || 'Erro desconhecido', data, userId);
      return { success: false, error: data?.error || 'Erro ao enviar e-mail', logged: true };
    }

    await logEmailDispatch(params, 'enviado', undefined, data, userId);
    return { success: true, data: data.data, logged: true };

  } catch (err: any) {
    await logEmailDispatch(params, 'erro', err.message, null, userId);
...
export async function dispatchPendenciaCritica(params: {
  destinatario: string;
  destinatario_nome: string;
  ...
  responsavel_id?: string;
  box_id?: string;
}, userId?: string): Promise<EmailDispatchResult> {
...
export async function dispatchVencimentoCertificado(params: {
  destinatario: string;
  destinatario_nome: string;
...
export async function dispatchGeracaoCobranca(params: {
  destinatario: string;
  destinatario_nome: string;
  box_codigo: string;
...
export async function dispatchAtualizacaoCadastro(params: {
  destinatario: string;
  destinatario_nome: string;
...
export async function dispatchNotificacaoPAD(params: {
  destinatario: string;
  destinatario_nome: string;
